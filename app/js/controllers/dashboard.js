  'use strict';
    ...
    app.config(function(ChartJsProvider) {
     ...
     app.controller('blogDetailsCtrl', ['$scope', '$timeout','$http', function($scope, $timeout,$http) {
            $http.get(sessionStorage.IonServer+"/index.php/request?action=approvedblogs&module=ionize&resource=posts&user_id="+sessionStorage.USER_ID+"&group_id="+sessionStorage.category).then(
                function(success){
                    $scope.approved_blogs=success.data.description.count;
                },function(error){});
                $http.get(sessionStorage.IonServer+"/index.php/request?action=totalincome&module=ionize&resource=posts&user_id="+sessionStorage.USER_ID+"&type=income").then(
                function(success){
                    $scope.total_income=success.data.description.amount;
                },function(error){})
                $http.get(sessionStorage.IonServer+"/index.php/request?action=totalincome&module=ionize&resource=posts&user_id="+sessionStorage.USER_ID+"&group_id="+sessionStorage.category).then(
                function(success){
                    console.log(success);
                    $scope.rating=success.data.description.rate;
                },function(error){})
    ...
    app.controller('DashboardLineCtrl', ['$scope', '$timeout','$http', function($scope, $timeout,$http) {
        $scope.labels=[];
        $scope.data=[];
        $scope.internal_array=[];
        console.log(sessionStorage.IonServer+"/index.php/request?action=monthdata&module=ionize&resource=posts&user_id="+sessionStorage.USER_ID+"&group_id="+sessionStorage.category);
        $http.get(sessionStorage.IonServer+"/index.php/request?action=monthdata&module=ionize&resource=posts&user_id="+sessionStorage.USER_ID+"&group_id="+sessionStorage.category)
        .then(function(success){
            for(var i=0;i<success.data.description.length;i++){
                $scope.labels.push(success.data.description[0].Month);
                $scope.internal_array.push(success.data.description[0].Count);
            }
            $scope.data.push($scope.internal_array);
        },function(error){})
    ...
    app.controller('DashboardBarCtrl', ['$scope', '$timeout','$http', function($scope, $timeout,$http) {
        $scope.options = {
            scaleShowVerticalLines: false
        };
        // $scope.labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep'];
